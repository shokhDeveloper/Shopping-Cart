...
export const Store: React.FC = (): JSX.Element => {
    ...
    useEffect(() => {
        ...
        } else if (pathname.substring(0, 10) === "/by-libary") {
            dispatch(setApplicationType("libary"))

        }
    }, [pathname])
    const handleAddCartLibary = (tovar: any): void => {
        addItem(tovar)
    }
    return (
        <>
    ...
    {(function () {
            if (applicationType === "libary") {
                console.log("ishladi")
      if (items.length) {
          if(items.some((tovar) => tovar.id.toString()  === item.id.toString())){
              let tovar = items.find((tovar) => tovar.id.toString() === item.id.toString())
              return(
                  <div className={CompModule.card__box}>
                      <Button onClick={() => updateItemQuantity(tovar?.id ? tovar.id: "", tovar?.quantity ? tovar.quantity + 1 : 0) } type="primary">Count +</Button>
              ...
              return (
                  <Button onClick={() => handleAddCartLibary(item)} className={CompModule.card__btn} type="primary">+ Add To Cart</Button>
              )    
          }
      } else {
          return (
